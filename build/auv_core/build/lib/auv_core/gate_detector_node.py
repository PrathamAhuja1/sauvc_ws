...
class GateDetectorNode(Node):
    ...
    def image_callback(self, msg: Image):
        ...
        # PRIORITY 1: Detect orange flare (must avoid!)
    ...
    def detect_orange_flare(self, hsv_image, debug_img):
        """Detect orange flare that MUST be avoided"""
        ...
        if largest_flare:
            ...
            if debug_img is not None:
                ...
                cv2.putText(debug_img, "ORANGE FLARE - AVOID!", 
                ...
                cv2.rectangle(debug_img, (danger_left, 0), 
                            (danger_right, self.image_height), 
                            (0, 0, 255), 2)
        
        return flare_info
    
    def process_flare_avoidance(self, flare_info, header, debug_img):
        """Publish flare avoidance commands"""
        # Publish flare detected
        flare_msg = Bool()
        flare_msg.data = True
        self.flare_detected_pub.publish(flare_msg)
        
        # Publish flare position
        pos_msg = PoseStamped()
        ...
        # If flare is on left (negative x), move right (positive)
        # If flare is on right (positive x), move left (negative)
        ...
        warning = String()
        if abs(flare_info['normalized_x']) < self.flare_danger_threshold:
    ...
    def process_gate_detection(self, red_bar, green_bar, flare_info, header, debug_img):
        ...
        if debug_img is not None:
            ...
            cv2.line(debug_img, (image_center_x, 0), 
                    (image_center_x, self.image_height), (255, 0, 255), 1)
            ...
            if flare_info['detected']:
                info_text.append("FLARE DETECTED - ADJUSTING")
